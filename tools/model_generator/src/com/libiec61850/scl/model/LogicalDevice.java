...
public class LogicalDevice {
	...
	public LogicalDevice(Node ldNode, TypeDeclarations typeDeclarations, IED ied) throws SclParserException {
	...
	private void parseLogicalNodes(Node ldNode, TypeDeclarations typeDeclarations) throws SclParserException {
		...
		logicalNodes.add(new LogicalNode(ln0Node, typeDeclarations, this.ied));
		
		List<Node> lnNodes = ParserUtils.getChildNodesWithTag(ldNode, "LN");
		
		for (Node lnNode : lnNodes) {
			logicalNodes.add(new LogicalNode(lnNode, typeDeclarations, this.ied));
		}
	}
